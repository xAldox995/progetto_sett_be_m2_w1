...
public interface PrenotazioniRepo extends JpaRepository<Prenotazione, Long> {
    ...
    @Query("SELECT p FROM Prenotazione p JOIN p.postazioni pos WHERE pos.id = :postazioneId AND p.data_prenotazione = :data")
    List<Prenotazione> findByPostazioneIdEData(long postazioneId, LocalDate data);

    @Query("SELECT p FROM Prenotazione p WHERE p.utente.id = :utenteId AND p.data_prenotazione = :data AND :postazioneId IN (SELECT pos.id FROM p.postazioni pos)")
